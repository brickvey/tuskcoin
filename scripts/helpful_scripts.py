...
# need to change on binance tokens
contract_to_mock = {
    # "eth_usd_price_feed": MockV3Aggregator,
    # "dai_usd_price_feed": MockV3Aggregator,
    "bnb_usd_price_feed": MockV3Aggregator,
    "doge_usd_price_feed": MockV3Aggregator,
    "wbnb_token": MockBNB,
    "doge_token": MockDOGE,
    # "fau_token": MockDAI,
    "weth_token": MockWETH,
...
def deploy_mocks(decimals=DECIMALS, initial_value=INITIAL_PRICE_FEED_VALUE):
    ...
    print("Deploying Mock BNB...")
    wbnb_token = MockBNB.deploy({"from": account})
    print(f"Deployed to {wbnb_token.address}")
    print("Deploying Mock DOGE...")
    doge_token = MockDOGE.deploy({"from": account})
    print(f"Deployed to {doge_token.address}")
